...
options:
  interface_alias:
    description:
      ...
      - If C(interface_alias) is not provided and C(gateway) is provided, the module uses the M(Find-NetRoute) cmdlet to determine the value of C(interface_alias).
    type: str
  destination:
    description:
  ...
  gateway:
    description:
        ...
        - If C(gateway) and C(interface_alias) are not provided it will be set to C(0.0.0.0).
        - If C(gateway) is not provided it will be set to the gateway defined for C(interface_alias).
    type: str
  metric:
    description:
